...
with open("/hdd1/changshuo/PRECISE_PROSPECTIVE_PROCESSED_20231212/synthstrip_dict.json","r") as json_file:
    pid_to_file=json.load(json_file)

# parse command line
parser = argparse.ArgumentParser()
parser.add_argument('-i', '--image', metavar='file', help='Input image to skullstrip.')
...
if args.model is not None:
    ...
    if args.no_csf:
        ...
        modelfile = "/hdd1/changshuo/cycleGAN/segmenter/synthstrip/synthstrip.nocsf.1.pt"
    else:
        modelfile = "/hdd1/changshuo/cycleGAN/segmenter/synthstrip/synthstrip.1.pt"
checkpoint = torch.load(modelfile, map_location=device)
model.load_state_dict(checkpoint['model_state_dict'])

# load input volume
for pid in pid_to_file.keys():

    image = sf.load_volume(pid_to_file[pid])
    image[image<-950]=-950
    image[image>1550]=1550
    image = ((image+950) / 2500 * 255).astype(np.uint8)
    file_name=pid
    print(f'Input image read from: {pid}')
    ...
    for i in range(0,image.shape[-1]):
        Image.fromarray(np.rot90(np.array(image[:,:,i]))).convert("L").save(os.path.join("/hdd1/changshuo/PRECISE_PROSPECTIVE_PROCESSED_20231212/synthstrip-result",str(pid.split("/")[-1])+"-"+str(i)+".jpg"))
