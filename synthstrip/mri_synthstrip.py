...
temp=np.array(Image.open(args.image).convert("RGB"))
# print(np.min(temp))
# temp[temp<-950]=-950
# temp[temp>1550]=1550
# temp=((temp+950)/2500 * 255).astype(np.uint8)
image = sf.Volume(temp)
...
if args.out:
    ...
    Image.fromarray(np.rot90(np.array(image))).convert("L").save(os.path.join(args.out,file_name))
